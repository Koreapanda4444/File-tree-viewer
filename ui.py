...
class App(ctk.CTk):
    ...
    def _configure_ttk(self):
        ...
        try:
            ...
            pass
        style.configure("Treeview", background="#1f2937", fieldbackground="#1f2937", foreground="#f3f4f6", rowheight=30, borderwidth=0, font=("Segoe UI", 11))
    ...
    def _toolbar(self, parent, is_vm: bool):
        ...
        ctk.CTkButton(bar, text=T.BTN_CLEAR_SEARCH, width=90, command=self._on_clear_search).grid(row=0, column=6, padx=(0, 12), pady=10)
    ...
    def _split(self, parent, is_vm: bool):
        ...
        ctk.CTkLabel(left, text=T.PANEL_EXPLORER, font=ctk.CTkFont(size=15, weight="bold")).grid(row=0, column=0, padx=12, pady=(10, 6), sticky="w")
        ...
        ctk.CTkLabel(right, text=T.PANEL_DETAILS, font=ctk.CTkFont(size=15, weight="bold")).grid(row=0, column=0, padx=12, pady=(10, 6), sticky="w")
        self._detail_rows = getattr(self, "_detail_rows", {})
        rows = {}
        box = ctk.CTkFrame(right)
        box.grid(row=1, column=0, sticky="ew", padx=10, pady=(0, 10))
        box.grid_columnconfigure(1, weight=1)
        for i,(k,lbl) in enumerate([("name",T.DETAIL_NAME),("path",T.DETAIL_PATH),("type",T.DETAIL_TYPE),("size",T.DETAIL_SIZE),("modified",T.DETAIL_MODIFIED)]):
        ...
        for idx,(txt,cmd) in enumerate([(T.BTN_RENAME,self._on_rename),(T.BTN_DELETE,self._on_delete),(T.BTN_COPY,self._on_copy),(T.BTN_MOVE,self._on_move)]):
        ...
        ctk.CTkLabel(right, text=T.PANEL_PREVIEW, font=ctk.CTkFont(size=15, weight="bold")).grid(row=2, column=0, padx=12, pady=(0, 6), sticky="nw")
        pbox = ctk.CTkFrame(right)
        pbox.grid(row=3, column=0, sticky="nsew", padx=10, pady=(0, 10))
        pbox.grid_rowconfigure(0, weight=1); pbox.grid_columnconfigure(0, weight=1)
        ptext = ctk.CTkTextbox(pbox); ptext.grid(row=0, column=0, sticky="nsew", padx=8, pady=8); ptext.configure(state="disabled")
        pimg = ctk.CTkLabel(pbox, text=""); pimg.grid(row=0, column=0, sticky="nsew", padx=8, pady=8); pimg.grid_remove()
    ...
    def _build_vm(self):
        ...
        for i in range(4): vmbar.grid_columnconfigure(i, weight=1)
        ...
        vm_body.grid_columnconfigure(0, weight=1); vm_body.grid_rowconfigure(0, weight=1)
    ...
    def _build_bottom(self):
        ...
        ctk.CTkLabel(bottom, text=T.PANEL_LOG, font=ctk.CTkFont(size=15, weight="bold")).grid(row=0, column=0, padx=12, pady=(10, 6), sticky="w")
        self.log_box = ctk.CTkTextbox(bottom, height=120); self.log_box.grid(row=1, column=0, sticky="ew", padx=12, pady=(0, 10)); self.log_box.configure(state="disabled")

        self.undo_bar = ctk.CTkFrame(bottom); self.undo_bar.grid(row=2, column=0, sticky="ew", padx=12, pady=(0, 10))
        self.undo_bar.grid_columnconfigure(0, weight=1)
        self.undo_label = ctk.CTkLabel(self.undo_bar, text="", anchor="w"); self.undo_label.grid(row=0, column=0, padx=10, pady=8, sticky="w")
        self.undo_btn = ctk.CTkButton(self.undo_bar, text=T.BTN_UNDO, width=120, command=self._on_undo); self.undo_btn.grid(row=0, column=1, padx=10, pady=8)
    ...
    def clear_details(self):
        target = self._detail_rows if self.tabs.get() == T.TAB_REAL else self._vm_detail_rows
        for k in target: target[k].configure(text="-")

    def clear_preview(self):
        self._set_tb(self.preview_text, "")
        self.preview_img.grid_remove(); self.preview_text.grid()
        self._preview_ref = None

        self._set_tb(self.vm_preview_text, "")
        self.vm_preview_img.grid_remove(); self.vm_preview_text.grid()
        self._vm_preview_ref = None
    ...
    def set_preview_image(self, filepath: str):
        if Image is None:
            ...
            return
        try:
            img = Image.open(filepath); img.thumbnail((680, 440))
            cimg = ctk.CTkImage(light_image=img, dark_image=img, size=img.size)
        except Exception as e:
            self.set_preview_text(f"(image preview error) {e}")
            return
        if self.tabs.get() == T.TAB_REAL:
            self.preview_text.grid_remove(); self.preview_img.grid()
            self.preview_img.configure(image=cimg); self._preview_ref = cimg
        else:
            self.vm_preview_text.grid_remove(); self.vm_preview_img.grid()
            self.vm_preview_img.configure(image=cimg); self._vm_preview_ref = cimg

    def prompt(self, title: str, message: str) -> Optional[str]:
        dlg = SimplePrompt(self, title, message); self.wait_window(dlg); return dlg.result

    def open_text_editor(self, title: str, filename: str, initial: str) -> Optional[str]:
        dlg = TextEditor(self, title, filename, initial); self.wait_window(dlg); return dlg.result
    ...
    def _load_children_async(self, parent_item, folder: Path, backend):
        def worker():
            ...
            def apply():
                try:
                    for c in self.tree.get_children(parent_item): self.tree.delete(c)
                except Exception:
                    return
    ...
    def _insert_vm(self, parent_item, node_id: str, backend):
        for c in self.vm_tree.get_children(parent_item): self.vm_tree.delete(c)
    ...
    # ------- search highlight / filter (simple highlight only in compact UI) -------
    def _apply_highlight(self, tree: ttk.Treeview, query: str):
        ...
        def walk(item):
            ...
            for c in tree.get_children(item): walk(c)
    ...
    def _on_tree_right_click(self, e):
        ...
        if it: self.tree.selection_set(it); self.tree.focus(it)
        if not hasattr(self, "_menu_real"): self._menu_real=self._make_menu(False)
        self._menu_real.tk_popup(e.x_root, e.y_root)

    def _on_vm_tree_right_click(self, e):
        it = self.vm_tree.identify_row(e.y)
        if it: self.vm_tree.selection_set(it); self.vm_tree.focus(it)
    ...
    # ------- drag & drop -------
    def _on_drag_start_real(self, e): self._drag_item_real=self.tree.identify_row(e.y)
    def _on_drag_end_real(self, e):
        if not self.controller: return
        src=self._drag_item_real; self._drag_item_real=None
        if not src: return
        dst=self.tree.identify_row(e.y)
        if not dst or dst==src: return
        sp=self._tree_item_to_payload.get(src); dp=self._tree_item_to_payload.get(dst)
        if not sp or not dp or not Path(dp).is_dir(): return
        self.tree.selection_set(src)
        self.controller.real_on_select_path(sp)
        self.controller.real_move_many([sp], Path(dp))

    def _on_drag_start_vm(self, e): self._drag_item_vm=self.vm_tree.identify_row(e.y)
    def _on_drag_end_vm(self, e):
        if not self.controller: return
        src=self._drag_item_vm; self._drag_item_vm=None
        if not src: return
        dst=self.vm_tree.identify_row(e.y)
        if not dst or dst==src: return
        sid=self._vm_tree_item_to_id.get(src); did=self._vm_tree_item_to_id.get(dst)
        if not sid or not did or not self.controller.vm.nodes[did].is_dir: return
    ...
    def _on_vm_tree_double_click(self, e):
        ...
        if not nid: return
        if self.controller.vm.nodes[nid].is_dir:
    ...
    def _real_selected_paths(self)->List[Path]:
        ...
        for it in self.tree.selection():
            p=self._tree_item_to_payload.get(it)
            if isinstance(p, Path): out.append(p)
        return out
    ...
    def _on_tab_changed(self):
        if self.controller: self.controller.on_tab_change(self.tabs.get())

    def _on_select_root_real(self):
        if self.controller: self.controller.select_root_real()

    def _on_refresh(self):
        if not self.controller: return
        if self.tabs.get()==T.TAB_REAL:
            self.controller.refresh_real()
            self._apply_highlight(self.tree, (self.real_search_entry.get() or "").strip())
        else:
            self.controller.refresh_vm()
            self._apply_highlight(self.vm_tree, (self.vm_search_entry.get() or "").strip())
    ...
    def _on_clear_search(self):
        (self.real_search_entry if self.tabs.get()==T.TAB_REAL else self.vm_search_entry).delete(0,"end")
        self._on_refresh()
    ...
    def _on_tree_select(self, _=None):
        ...
        if not sel: return
        p=self._tree_item_to_payload.get(sel[0])
        if p: self.controller.real_on_select_path(p)

    def _on_tree_open(self, _=None):
        if not self.controller or not self.controller.real.root: return
        it=self.tree.focus()
        p=self._tree_item_to_payload.get(it)
        if not p or not isinstance(p, Path) or not p.is_dir(): return
    ...
    def _on_vm_tree_select(self, _=None):
        ...
        if not sel: return
        nid=self._vm_tree_item_to_id.get(sel[0])
        if nid: self.controller.vm_on_select_node(nid)
    ...
    def _on_rename(self):
        ...
        if self.tabs.get()==T.TAB_REAL:
            ...
            if not ps: return
            self.controller.real_on_select_path(ps[0]); self.controller.real_rename()
        else:
            ids=self._vm_selected_ids()
            if not ids: return
            self.controller.vm_on_select_node(ids[0]); self.controller.vm_rename()
    ...
    def _on_undo(self):
        if self.controller: self.controller.undo()

    def _on_vm_save(self):
        if self.controller: self.controller.vm_save()
    def _on_vm_load(self):
        if self.controller: self.controller.vm_load()
    def _on_vm_clear(self):
        if self.controller: self.controller.vm_clear()
    def _on_vm_export(self):
        if self.controller: self.controller.vm_export_to_real()

    def vm_path_to_id(self, vm_path: str, backend) -> Optional[str]:
        if not vm_path.startswith("VM:/"): return None
